...
import { Form, Input, Row, Col, notification, message, Checkbox } from 'antd'
import './style.css'
//提示框
...
class LoginForm extends React.Component {
  ...
  //         // message.success('欢迎回来'+isAuthenticated());
  ...
  //    //判断是否已经登录过
  //    if (localStorage.getItem('user')) {
  //     message.success('登录成功 | 欢迎回来 - '+ isAuthenticated());
...
class Login extends React.Component {
  ...
  initPage = () => {
    ...
    this.loadImageAsync(url).then(url => {
      ...
      this.particle.init()
      notification.open({
        message: <ul><li>账号密码请联系管理员</li></ul>,
        duration: 0,
